<template>
  <div>
    ...
    <h2>Item</h2>
    <!-- 
      v-once 속성을 추가한 경우, props 로 전달되는 값 유형(객체, 원시값) 변경에 따라 영향도가 다름
        - 원시값
          - v-once 속성이 있을 경우, 한번 주입된 후 상위 컴포너트에서 props 를 변경해도 해당 값 변경에 영향을 받지 않음 
          - 즉, 상위 컴포넌트에서 하위 컴포넌트 props 를 통해 값 주입 후, 이후 값을 변경해도, v-once 가 설정된 컴포넌트는 변경된 값 반영안됨
        - 객체
          - 객채({})의 메모리 주소가 변경될 경우(불변성유지 값 변경) v-once 속성이 있을 경우 값 변경에 대한 영향을 받지 않음
          - 객체의 메모리 주소가 변경되지 않는다면 v-once 속성이 있어도 값 변경의 영향을 받음!
          - v-once 가 없을 경우, 불변성을 유지하던 유지하지 않던, 상위 컴포넌트에서 객체가 변경되면 무조건 props 로 넘기는 하위 컴포넌트 영향받음
    -->
